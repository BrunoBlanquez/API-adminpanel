...
class ProdutosController {
  ...
  static listarProdutoId = (req, resp) => {
    ...
    .exec((erro, produtos) => {
      erro ? resp.status(400).send({message: `${erro.message} - ID não localizado no banco de dados`}) : resp.status(200).send({_id: produtos._id, nome: produtos.nome, imagem: produtos.imagem, estoque: produtos.estoque, preco: produtos.preco, marca: produtos.marca})
    })
  }
